"""Shared building blocks for the planning_node_* variants.

All four planning nodes (vanilla / default / turn / centerline) share the same
occupancy-grid + ESDF + trajectory-library pipeline. The only differences are:

  - how the goal used in the cost function is resolved (`_resolve_target`), and
  - an optional extra per-trajectory cost term (`_extra_cost`).

Variants subclass `PlanningNodeBase` and override those two hooks.
"""
...
def generate_trajectory_library_3d(
    ...
    omega_y_samples = np.linspace(-np.pi / 2, np.pi / 2, num_samples)
...
# === PlanningNode base class ===
class PlanningNodeBase(Node):
    """Shared occupancy-grid + ESDF + trajectory-library planner.

    Subclasses customize behaviour by overriding `_resolve_target` (the goal
    used in the cost function) and `_extra_cost` (an additive per-trajectory
    cost term). Extra publishers/subscribers can be added by overriding
    `_setup_extras`, called at the end of `__init__`.
    """
    ...
    def _setup_extras(self):
        """Extra publishers/subscribers/state for a variant. Default: none."""
        pass

    def _resolve_target(self, T, ESDF_map, depth_msg):
        """Goal position used in the cost function. Default: the raw target_pose."""
        return self.target_pose

    def _extra_cost(self, traj, ESDF_map):
        """Additive per-trajectory cost term. Default: none."""
        return 0.0
